;(function (window, document) {
    ...
    if (self.webcam.readyState === self.webcam.HAVE_ENOUGH_DATA) {
      ...
      for (var i = 0; i < angles.length; i++) {
        ...
        for (var currentRect = 0; currentRect < tempRects.length; currentRect++){
            ...
            //tempRects[currentRect].x = tempRects[currentRect].x + r * Math.cos(angles[i]);
            //tempRects[currentRect].y = tempRects[currentRect].y + r * Math.sin(angles[i]);
  ...
  FaceDetect.prototype.drawFaces = function(rects, sc, max) {
    ...
    for(var i = 0; i < n; ++i) {
        ...
        self.ctx.strokeRect((r.x) | 0, (r.y) | 0, (r.width) | 0, (r.height) | 0);
        //self.ctx.strokeRect((r.x*sc) | 0, (r.y*sc) | 0, (r.width*sc) | 0, (r.height*sc) | 0);
        self.ctx.rotate(-r.angle)
        self.ctx.translate(-x, -y);
